...
/**
 * 이웃수 패턴의 출현 주기성을 분석합니다.
 * 특정 기간 동안 이웃수가 나오지 않았을 때, 다음 회차에 나올 확률에 가중치를 부여합니다.
 */
public class NeighborPattern implements PatternAnalyzer {
    ...
    public void analyze(List<LottoPatternAnalyzer.LottoDraw> drawHistory) {
        ...
        for (int i = 0; i < drawHistory.size() - 1; i++) {
            ...
            boolean hasNeighbor = !java.util.Collections.disjoint(nextDraw.getWinningNumbers(), neighbors);
            hits.add(hasNeighbor);
        }
    ...
    public double getScore(int number, LottoPatternAnalyzer.LottoDraw latestDraw, List<LottoPatternAnalyzer.LottoDraw> drawHistory) {
        // 1. 현재 번호가 최신 회차의 이웃수 후보인지 확인
        Set<Integer> latestNeighbors = new HashSet<>();
        for (int num : latestDraw.getWinningNumbers()) {
            if (num > 1) latestNeighbors.add(num - 1);
            if (num < 45) latestNeighbors.add(num + 1);
        }
        if (!latestNeighbors.contains(number)) {
            return 0;
        }
        ...
        if (currentMissStreak > 0) {
            ...
            if (stats != null && stats[1] >= MIN_OCCURRENCES_FOR_STAT) {
                ...
                // 이웃수 패턴은 매우 자주 발생하므로 가중치를 낮게 설정 (200 -> 250, 15.0 -> 20.0)
                if (probability > 0.5) {
                    return (probability * 250) + (currentMissStreak * 20.0);
                }
            }
        }
